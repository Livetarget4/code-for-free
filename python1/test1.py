...
class AdventureGame:
    ...
    def load_game(self):
        ...
        try:
            ...
            # Split the data
            name, char_class, level, gold = data.split(",")
            
            # Apply the data WITHOUT calling create_character
            self.player["name"] = name
            self.player["class"] = char_class
            self.player["level"] = int(level)
            self.player["gold"] = int(gold)
            
            # Set up the class-specific attributes
            class_data = self.classes[char_class]
            self.player["hp"] = class_data["base_hp"]
            self.player["max_hp"] = class_data["base_hp"]
            self.player["mp"] = class_data["base_mp"]
            self.player["max_mp"] = class_data["base_mp"]
            self.player["skills"] = class_data["skills"]
            
            print(f"✓ Successfully loaded game for {name}!")
            return True
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == "1":
            game.create_character()  # Only create character for new game
            game.play()
            break
        elif choice == "2":
            print("\nAttempting to load game...")
            if game.load_game():
                print("Starting loaded game...")
                game.play()
                break
            else:
                print("\nLoad failed. Would you like to:")
                print("1. Try loading again")
                print("2. Start new game")
                retry = input("Choose option: ")
                if retry == "2":
                    game.create_character()  # Create character if load fails
                    game.play()
                    break
        elif choice == "3":
            print("\nThanks for playing!")
